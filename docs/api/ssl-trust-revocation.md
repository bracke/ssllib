# ssl-trust-revocation

Generated from `src/ssl-trust-revocation.ads` by `ssllib_tools docs`. The prose is the specification's own; nothing here is written twice.

Revocation status: what the policy demands, and where the evidence
comes from.
