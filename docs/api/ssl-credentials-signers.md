# ssl-credentials-signers

Generated from `src/ssl-credentials-signers.ads` by `ssllib_tools docs`. The prose is the specification's own; nothing here is written twice.

External signers: a signing capability this library calls but does
