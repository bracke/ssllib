...
Generated from `src/ssl-exporters.ads` by `ssllib_tools docs`. The prose is the specification's own; nothing here is written twice.

Exported keying material: keys for a protocol layered on top of
