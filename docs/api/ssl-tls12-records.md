...
Generated from `src/ssl-tls12-records.ads` by `ssllib_tools docs`. The prose is the specification's own; nothing here is written twice.

The TLS 1.2 record layer, which is a different construction from
