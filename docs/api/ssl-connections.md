...
Generated from `src/ssl-connections.ads` by `ssllib_tools docs`. The prose is the specification's own; nothing here is written twice.

A TLS connection: an engine with a transport bound to it, and the
