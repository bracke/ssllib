...
Generated from `src/ssl-diagnostics.ads` by `ssllib_tools docs`. The prose is the specification's own; nothing here is written twice.

Structured diagnostic events, and the sink an application supplies
to receive them.
