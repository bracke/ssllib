...
private package SSL.Secrets is
   ...
   --  The widest secret this library holds in one of these: 66 octets is the
   --  P-521 ECDH shared secret, 64 the SHA-512 output width, 48 the TLS 1.2
   --  master secret and the SHA-384 key-schedule width. The bound is fixed
   --  rather than a discriminant so that a secret is a plain component of the
   --  state that owns it and no derivation step allocates.
   Maximum_Length : constant Byte_Index := 128;

   subtype Secret_Length is Byte_Index range 0 .. Maximum_Length;

   type Secret is tagged limited private;
   ...
   procedure Set (Item : in out Secret; Data : Byte_Array)
     with Pre => Data'Length <= Maximum_Length,
   ...
   procedure Set_Length (Item : in out Secret; Length : Secret_Length)
     with Post => SSL.Secrets.Length (Item) = Length;
   ...
   procedure Copy (Target : in out Secret; Source : Secret)
     with Post => Length (Target) = Length (Source);
...
private
   ...
   subtype Octet_Buffer is Byte_Array (1 .. Maximum_Length);

   type Secret is new Ada.Finalization.Limited_Controlled with record
      --  The buffer is always full width. Only the first Used octets are the
      --  secret; the rest are zero, and Wipe restores that.
      Octets : Octet_Buffer := [others => 0];
