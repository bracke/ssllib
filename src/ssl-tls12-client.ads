...
--  @summary The restricted TLS 1.2 client handshake, as an explicit state
--  machine.
--
...
private
   ...
   type Machine is limited record
      ...
      --  The transcript. TLS 1.2 hashes the same octets TLS 1.3 does -- type,
      --  three-octet length, body -- so the same package serves, under the
      --  suite's own hash.
      ...
      --  RFC 7627. Set only when the server echoed the extension, and checked
      --  before anything is derived: without it the master secret is not bound
      --  to this handshake.
      Extended_Master : Boolean := False;

      Certificate_Requested : Boolean := False;
      Peer_Accepted         : Boolean := False;

      --  The server's Finished verify data, kept so that it can be checked
      --  against what this end computes once the transcript reaches it.
      Server_Verify : Byte_Array (1 .. Verify_Data_Length) := [others => 0];

      --  RFC 5077.
      Wants_Tickets : Boolean := False;
      Offered       : SSL.Sessions.Session;
      Has_Offer     : Boolean := False;
      Is_Resumed    : Boolean := False;

      --  Set from the ServerHello: the server promised a NewSessionTicket, so
      --  one is expected and anything else is not.
      Expect_Ticket : Boolean := False;

      New_Ticket        : Byte_Array (1 .. SSL.Sessions.Maximum_Ticket) := [others => 0];
      New_Ticket_Length : Byte_Index range 0 .. SSL.Sessions.Maximum_Ticket := 0;
      New_Lifetime      : Natural := 0;
      Has_New_Ticket    : Boolean := False;
   ...
   function Peer_Scheme (Item : Machine) return SSL.Signature_Schemes.Signature_Scheme is
     (Item.Scheme);
   function Peer_Certificate (Item : Machine)
   ...
   function Offers_Ticket (Item : Machine) return Boolean is (Item.Wants_Tickets);
