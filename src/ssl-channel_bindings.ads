...
--  @summary Channel bindings: a value that ties an authentication exchange
...
package SSL.Channel_Bindings is

   --  A tls-exporter binding is 32 octets by definition (RFC 9266 section 2).
   Exporter_Binding_Length : constant Byte_Index := 32;

   --  A tls-server-end-point binding is the certificate hash, which this
   --  library always takes with SHA-256.
   End_Point_Binding_Length : constant Byte_Index := 32;
