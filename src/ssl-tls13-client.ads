...
--  @summary The TLS 1.3 client handshake, as an explicit state machine.
--
...
private
   ...
   Maximum_Cookie : constant Byte_Index := 1024;
   Maximum_Context : constant Byte_Index := 255;

   type Pair_Array is
     array (1 .. SSL.Handshake_Messages.Maximum_Offered_Shares)
       of SSL.Crypto.Key_Exchange_Pair;

   type Machine is limited record
      Config : access constant SSL.Configurations.Client_Configuration;
      ...
      State   : Client_State := Start;

      --  Preserved across a HelloRetryRequest. RFC 8446 section 4.1.2 requires
      --  the second ClientHello to keep the random, the legacy session
      --  identifier, the server name, the protocols, the suites and the
      --  versions -- everything except the key share the retry asked to change.
      Random_Value : SSL.Handshake_Messages.Random_Bytes := [others => 0];
      Session      : Byte_Array (1 .. 32) := [others => 0];

      --  One ephemeral keypair per offered share. They are held here rather
      --  than in the shared context because a client offers several and a
      --  server generates one, and every one of them is scrubbed the moment
      --  the server has picked -- including the ones it did not pick.
      Pairs         : Pair_Array;
      Offered       : SSL.Handshake_Messages.Key_Share_List;
      Offered_Count : Natural range 0 .. SSL.Handshake_Messages.Maximum_Offered_Shares := 0;

      Cookie_Length : Byte_Index := 0;
      Cookie        : Byte_Array (1 .. Maximum_Cookie) := [others => 0];

      Asked_For_Certificate : Boolean := False;
      Sent_A_Certificate    : Boolean := False;
      Context_Length        : Byte_Index := 0;
      Request_Context       : Byte_Array (1 .. Maximum_Context) := [others => 0];

      --  What the server said it would accept a signature under. Kept, because
      --  the answer is written several messages later -- after the server's
      --  Finished -- and by then the request is out of reach.
      Requested_Schemes     : SSL.Signature_Schemes.Scheme_List;

      Peer : SSL.Certificate_Validation.Validation_Result;

      --  The session this client is offering, and whether the offer actually
      --  went into the ClientHello. A session that was looked up but not
      --  offered -- because the suite could not be committed to, say -- must
      --  not be treated as accepted when a server happens to send back a
      --  pre_shared_key.
      Offer           : SSL.Sessions.Session;
      Offered_Session : Boolean := False;
      Resumed         : Boolean := False;

      --  RFC 5077's ticket, carried in this hello for the TLS 1.2 machine that
      --  may adopt it. Only the octets: the session itself belongs to whoever
      --  will use it if the server selects TLS 1.2, and this machine never
      --  derives anything from it.
      Legacy_Ticket        : Byte_Array (1 .. SSL.Sessions.Maximum_Ticket) := [others => 0];
      Legacy_Ticket_Length : Byte_Index range 0 .. SSL.Sessions.Maximum_Ticket := 0;
      Offer_Legacy         : Boolean := False;

      --  Set the moment the peer's certificate is accepted, so that
      --  CertificateVerify cannot be processed before it.
      Peer_Certificate_Accepted : Boolean := False;

      --  The read and write traffic states belong to the engine, not here; the
      --  machine only says when to install into them. These two record what has
      --  been installed so that a second install cannot happen by accident.
      Handshake_Keys_Installed   : Boolean := False;
      Application_Keys_Installed : Boolean := False;
   end record;

   function State_Of (Item : Machine) return Client_State is (Item.State);
   ...
   function Was_Asked_For_Certificate (Item : Machine) return Boolean is
     (Item.Asked_For_Certificate);
   function Sent_Certificate (Item : Machine) return Boolean is (Item.Sent_A_Certificate);
   function Resumed (Item : Machine) return Boolean is (Item.Resumed);
   function Peer_Certificate (Item : Machine)
     return SSL.Certificate_Validation.Validation_Result is (Item.Peer);
